...
- 允许列表中的外链图片（当前包括 Gitee、知乎图床、CSDN、GitHub、Uploadcare）会在同步时下载到临时快照，再通过 WordPress 媒体库导入；文章最终只保存本站媒体 URL，避免第三方防盗链或链接失效。其他主机、缺失、越界或不支持的图片会在正文中显示“图片未同步”占位并记录警告，不阻断其他笔记。单张仍不超过 10 MiB，SVG 不导入。
