...
`config/blog-sync.yml` 默认自动扫描 `wiki/` 下所有 Markdown，并按第一层子目录创建 WordPress 分类；`生活杂项`、`求职面试` 被排除。只读取已推送至 GitHub 的 wiki Markdown 与其本地图片；不改 wiki，不同步私密目录，不删除博客文章。首次只建草稿，请登录 WordPress 后台「文章 → 草稿」审阅后发布。
