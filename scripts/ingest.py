...
def build_prompt(fm, body, search_text=""):
    ...
    rules = rules_snippet()
    template = """## 角色设定
你是一位资深技术架构师，专注跟踪两大方向的前沿信息：

1. **AI Agent 开发**：RAG（检索增强生成）、Agent 工程化（harness / 编排 / 上下文管理 / 调度）、多智能体架构、Agent 评测、Agent 工具与平台（如 LangChain/LangGraph、Claude Code、Codex 生态）。
2. **跨平台开发**：Kotlin 多平台（KMP / Compose Multiplatform）、Flutter，以及共享逻辑 + 多端 UI 的落地架构。

你的任务是把【原始素材】加工为深度技术笔记，让读者 15 分钟内理解核心原理，并能在实际生产项目中应用。素材只要与上述两个方向相关，无论来源（论文 / 博客 / 开源项目 / 视频）都值得加工；与这两个方向无关但确有技术价值的内容可归入「通用技术」分类，不强行套用 Agent/跨平台视角。

## 质量门槛（借鉴深度调研的过滤标准）

先判断素材是否值得加工，再动手写。**必须满足（全部）**：
- 有实质技术内容（非纯营销 / 产品公告 / 软文）
- 有原创洞察、方案或可复现的信息（非简单转述他人结论）
- 提供可信来源（署名作者 / 官方文档 / 可验证链接），能支撑 [补充] 溯源

**加分项（满足越多，条目质量越高）**：
- 有实验数据 / 基准结果 / 成本对比
- 有可运行的最小代码示例或架构图
- 直面失败案例、反模式或权衡取舍
- 是近期（2 周内）的新进展，或为陈旧认知提供了更新

**直接排除**：
- 只有标题没有实质内容的空壳
- 纯榜单、纯招聘、与知识库无关的娱乐内容
- 与知识库已有条目高度重复、无增量信息（见下方「已知条目」）

若素材**不满足必须项**（如纯营销 / 极度浅薄 / 明确无关），不要强行生成笔记，而是输出：
  "skip": true, "skip_reason": "一句话说明为何不值得加工"
...
3. 人话解释：禁止直接复制教程化表述，所有核心技术概念必须给"一句话人话解释"（生活化类比或底层逻辑推演）。
4. 代码规范：涉及代码必须提供生产级最小可运行示例（含异常捕获、依赖版本锁定、安全边界），明确标注语言、框架及适用版本（如 "Kotlin 2.0 + KMP 1.9" 或 "langchain4j 2.x"）。
...
- **概念名称**
  - 🗣️ 人话：通俗类比
  - 🔧 本质：底层原理一句话
  - 📍 定位：AI Agent（RAG/Agent工程/调度）或 跨平台（KMP/Flutter）哪一环
  - 💡 补充：[补充]（附官方链接）
### 架构与方案对比（若有选型/架构内容）
...
## 知识库现有条目（「去重权威」：不得创建主题高度重复的条目；新增内容必须相对这些已有条目提供增量，并在「知识关联地图」中引用它们）
{entries[:4000]}
...
def process_one(p, dry_run, force, no_search):
    ...
    if data.get("skip"):
        reason = data.get("skip_reason") or "未提供原因"
        fm["status"] = "rejected"
        rewrite_fm(p, fm, body)
        return None, f"LLM 判定跳过：{reason}"
    if not data.get("entry"):
        return None, "LLM 输出无 entry，跳过"
