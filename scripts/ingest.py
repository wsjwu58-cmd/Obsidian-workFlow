...
2. 来源溯源：正文中融入补充内容时，必须在句末或段落末尾使用 [补充] 标签显式标识，区分原文观点与检索新知。
3. 人话解释：禁止直接复制学术化表述，所有核心技术概念必须给"一句话人话解释"（生活化类比或底层逻辑推演）。
4. 代码规范：涉及代码必须提供最小可运行示例，明确标注语言、框架及适用版本（如 "Kotlin 2.0 + KMP 1.9"）。
...
### 核心概念拆解
- 概念名称 / 人话解释 / 技术本质 / 技术栈定位（Agent/后端/KMP/Android 哪一环）/ 深度补充（[补充]）
### 架构与方案对比（若有选型/架构内容）
| 维度 | 方案A | 方案B | 方案C |（适用场景/核心优势/主要劣势/生产级成熟度/架构师推荐结论；[补充] 内容在单元格标注）
### 代码与实操速查
- 最小示例（可运行，[补充] 生成需标注）/ 关键配置（核心参数及含义）/ 常见报错与解决（Top 3）
### 避坑清单（Anti-patterns）
- 错误做法 → 正确做法（原因）
### 知识关联地图
- 前置知识 / 横向关联（Agent/后端/跨端交叉点）/ 纵向延伸（下一步方向 + 具体资源名称）
### 本周素材盲区与知识增量
- 原文盲区 / 知识增量总结（2-3 条额外收获）
### 本周行动清单
- [ ] 行动描述（预计耗时：xx分钟，关联知识点：xxx）"""
