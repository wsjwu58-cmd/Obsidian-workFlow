...
| 章节 | 主题 | 关键概念 |
|---|---|---|
| [s01](s01 Agent Loop：一个循环就够了.md) | Agent Loop | `messages` / `while True` / `stop_reason` |
| [s02](s02 Tool Use：多加一个工具，只加一行.md) | Tool Use | `TOOL_HANDLERS` / dispatch map / 并发 |
| [s03](s03 Permission：执行前做权限判断.md) | Permission | `PermissionRule` / 审批管线 |
| [s04](s04 Hooks：挂在循环上，不写进循环里.md) | Hooks | `PreToolUse` / `PostToolUse` / 扩展点 |
| [s05](s05 TodoWrite：没有计划的 Agent，做着做着就偏了.md) | TodoWrite | `TodoItem` / 先计划后执行 |
| [s06](s06 Subagent：大任务拆小，每个拿到的都是干净上下文.md) | Subagent | `fresh messages[]` / 上下文隔离 |
| [s07](s07 Skill Loading：用到的时候才加载.md) | Skill Loading | `SkillManifest` / 按需注入 |
| [s08](s08 Context Compact：上下文总会满，要有办法腾地方.md) | Context Compact | snip / micro / budget / auto 四层压缩 |
| [s09](s09 Memory：压缩会丢细节，要有一层不丢的.md) | Memory | selection / extraction / consolidation |
| [s10](s10 System Prompt：运行时组装，不硬编码.md) | System Prompt | 运行时组装 / 分段拼接 |
| [s11](s11 Error Recovery：错误不是结束，是重试的开始.md) | Error Recovery | token 升级 / fallback 模型 / 重试策略 |
| [s12](s12 Task System：目标太大，拆成小任务.md) | Task System | `TaskRecord` / `blockedBy` / 磁盘持久化 |
| [s13](s13 Background Tasks：慢操作放后台.md) | Background Tasks | 线程执行 / 通知队列 |
| [s14](s14 Cron Scheduler：按时间表生产工作.md) | Cron Scheduler | 持久化调度 / 会话级触发 |
| [s15](s15 Agent Teams：一个搞不定，组队来.md) | Agent Teams | `MessageBus` / 收件箱 / 权限冒泡 |
| [s16](s16 Team Protocols：队友之间要有约定.md) | Team Protocols | 关机握手 / 计划审批 |
| [s17](s17 Autonomous Agents：自己看板，自己认领.md) | Autonomous Agents | 空闲循环 / 自动认领 |
| [s18](s18 Worktree Isolation：各干各的，互不干扰.md) | Worktree Isolation | `WorktreeRecord` / 任务-目录绑定 |
| [s19](s19 MCP Tools：外接工具，标准协议.md) | MCP Plugin | 多传输 / 通道路由 / 工具池组装 |
| [s20](s20 Comprehensive Agent：全部机制，归到一个循环.md) | Comprehensive Agent | 全部机制归到一个循环 |
