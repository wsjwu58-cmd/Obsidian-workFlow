...
> 全库共 255 个 Markdown 文件（2026-09-14 更新：补入 wiki/AI与LLM/Agent-claudecode 系列 21 篇与 198. 打家劫舍 1 篇——原文件因 .git 损坏事件丢失，已由用户恢复重新入库）。关系总览见 [[知识图谱]]。
...
- [[DP动态规划]]：动态规划与状压 DP
- [[198. 打家劫舍 - 力扣（LeetCode）]]：LeetCode 198 打家劫舍——线性 DP 入门经典（选/不选相邻约束）
...
### Agent-claudecode（Learn Claude Code 系列，21 篇）

- [[00 Learn Claude Code 总览]]：系列总览——用 20 个机制拆解 Claude Code 的 Agent 形态与阅读路线
- [[s01 Agent Loop：一个循环就够了]]：Agent Loop 核心循环——LLM 决策与工具执行的交替闭环
- [[s02 Tool Use：多加一个工具，只加一行]]：工具注册与调用——扩展点极简的插件式工具体系
- [[s03 Permission：执行前做权限判断]]：权限模型——执行前拦截与用户确认的关卡设计
- [[s04 Hooks：挂在循环上，不写进循环里]]：Hooks 机制——生命周期挂点与循环解耦的扩展方式
- [[s05 TodoWrite：没有计划的 Agent，做着做着就偏了]]：TodoWrite 计划机制——让 Agent 自维护任务清单防偏航
- [[s06 Subagent：大任务拆小，每个拿到的都是干净上下文]]：Subagent 拆解——子代理隔离上下文处理大任务
- [[s07 Skill Loading：用到的时候才加载]]：技能按需加载——懒加载降低上下文占用
- [[s08 Context Compact：上下文总会满，要有办法腾地方]]：上下文压缩——满载时的腾挪与摘要策略
- [[s09 Memory：压缩会丢细节，要有一层不丢的]]：记忆系统——压缩之外的不丢失持久层
- [[s10 System Prompt：运行时组装，不硬编码]]：系统提示词运行时组装——动态拼装而非硬编码
- [[s11 Error Recovery：错误不是结束，是重试的开始]]：错误恢复——失败重试与降级路径
- [[s12 Task System：目标太大，拆成小任务]]：任务系统——目标分解为可执行小任务的 DAG
- [[s13 Background Tasks：慢操作放后台]]：后台任务——慢操作异步化不阻塞主循环
- [[s14 Cron Scheduler：按时间表生产工作]]：定时调度器——按时间表生产工作项
- [[s15 Agent Teams：一个搞不定，组队来]]：Agent Teams——多智能体组队协作拓扑
- [[s16 Team Protocols：队友之间要有约定]]：团队协议——队友间的消息约定与分工规则
- [[s17 Autonomous Agents：自己看板，自己认领]]：自治智能体——看板模式自领任务
- [[s18 Worktree Isolation：各干各的，互不干扰]]：Worktree 隔离——并行工作的 git 工作树隔离
- [[s19 MCP Tools：外接工具，标准协议]]：MCP 外接工具——标准协议接入外部能力
- [[s20 Comprehensive Agent：全部机制，归到一个循环]]：综合实战——全部机制收敛回一个循环的完整 Agent
