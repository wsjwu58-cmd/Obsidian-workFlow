...
- **描述**：按 `agents.md` 的 Ingest 规范执行：读取 `raw/` 待处理素材 → LLM 按**六维加工框架**提炼（结构化提炼 / 深度追问 / 联想缝合 / 场景转译 / 媒介转换 / 元数据标签）→ 判断分类 → 增量更新已有条目或新建条目 → 建立双向链接 → 更新 `index.md` / `知识图谱.md` / `log.md` → 标记素材已处理。
...
摄入外部素材时，AI 生成条目按六个维度加工（详见 `agents.md`「知识条目格式」）：

| 维度 | 产出 |
|------|------|
| 1. 结构化提炼 | `## 检索问题（Q&A）` + `## 结构化提炼`（核心论点 / 逻辑骨架 / 关键概念） |
| 2. 深度追问 | `## 深度追问`（苏格拉底式质疑 / 背景盲区 / 溯源验证） |
| 3. 联想与缝合 | `## 联想与缝合`（跨学科类比 / 知识库碰撞 / 底层模型） |
| 4. 场景化转译 | `## 场景化转译`（行动清单 / 避坑指南） |
| 5. 媒介转换 | Mermaid 图 / 表格 |
| 6. 元数据与检索 | 多维标签（type / status / 情绪）+ Q&A 前置 |

> 个人手写笔记仍保持原文不变，仅追加 `## 相关条目`；六维框架只用于 AI 从外部素材生成的新条目。
