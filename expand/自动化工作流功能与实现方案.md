...
| 环节           | 执行方                                  | 机制                         |
| ------------ | ------------------------------------ | -------------------------- |
| 版本管理 / 备份    | git + Obsidian Git 插件                   | 云端仓库 = 安全网                 |
| 定时采集         | GitHub Actions                       | `collect.yml` 每周一 06:00（GitHub/HN/ArXiv/RSSHub） |
| 过滤去重         | GitHub Actions（脚本）                   | 关键词 + URL/SimHash + LLM 打分 |
| AI 加工 Ingest | Codex 定时任务（已落地：每周一 06:30）或 Actions `ingest.yml` | 遵循 `agents.md` Ingest 规范 |
| 人工审阅         | 用户                                   | PR review / 每周抽查           |
| 每日巡检 Lint    | GitHub Actions + Codex 定时任务          | `lint.yml` 每天 08:30；Codex 日巡检每天 09:00 |
| 凭据扫描         | GitHub Actions                       | `scan-secrets.yml` 每次 push |
| 周报生成         | GitHub Actions                       | `weekly-report.yml` 每周五 19:00 |
| 失败告警         | GitHub Actions                       | `failure-notify.yml`：任一 workflow 失败即通知 |
| 语义检索         | 本地 Python + SQLite                   | `scripts/semantic_search.py`（embedding / TF-IDF 降级） |
...
flowchart LR
    ...
    subgraph GH["GitHub 私有仓库"]
        R["wiki/ + expand/ + raw/ + scripts/"]
        A1["collect.yml 采集过滤"]
        A2["ingest.yml AI 加工（可选）"]
        A3["lint.yml 每日巡检"]
        A4["scan-secrets.yml 凭据扫描"]
        A5["weekly-report.yml 周报"]
        A6["failure-notify.yml 失败告警"]
    end
    subgraph AI["AI 层"]
        CX["Codex 定时任务（周一加工/每日巡检）"]
        LLM["LLM API（打分/总结）"]
    end
    subgraph LOCAL["本地工具"]
...
- **输入/输出**：仓库 = `raw/` + `wiki/` + `expand/` + `scripts/` + `agents.md`（不含 `私密/`）。
...
- **全文抓取**：按类型抓取完整内容——GitHub 仓库 README（API）、文章正文（trafilatura）、arXiv（ar5iv）、YouTube / B 站字幕、PDF 文本。
- **中文翻译**：抓取内容经 DeepSeek API 翻译为简体中文，素材以中文 Markdown 保存（frontmatter：`source / url / title / collected / status / score / tags / translated`）。
- **验收**：每轮产出 5-30 条，素材含中文全文，通过率 20-50%（宁缺毋滥）。
...
- **描述**：按 `agents.md` 的 Ingest 规范执行：读取 `raw/` 待处理素材 → LLM 按**深度技术笔记模板**加工（核心概念拆解 / 方案对比 / 代码速查 / 避坑清单 / 知识关联地图 / `[补充]` 溯源）→ 判断分类 → 增量更新已有条目或新建条目 → 建立双向链接 → 更新 `index.md` / `知识图谱.md` / `log.md` → 标记素材已处理。
...
- **输出**：追加到 `expand/log.md` 或独立 `知识库周报.md`。
- **验收**：每周自动产出一份可读报告。
...
- **描述**：将 `wiki/` 与 `expand/` 向量化建立 embedding 索引（本地 sentence-transformers + SQLite），Query 按语义而非关键词检索。
- **验收**：同义 / 近义查询能命中相关条目。
...
├── .github/workflows/
│   ├── collect.yml                 # 采集 + 过滤
│   ├── ingest.yml                  # AI 加工（模式 B：深度技术笔记模板 + PR 审阅）
│   ├── lint.yml                    # 每日巡检
│   ├── scan-secrets.yml            # 凭据扫描
│   ├── weekly-report.yml           # 周报（周五 19:00）
│   └── failure-notify.yml          # 失败告警（webhook）
...
│   ├── weekly_report.py            # 周报
│   ├── semantic_search.py          # 语义检索（F11，embedding + TF-IDF 降级）
│   ├── codex_task.ps1              # Codex 定时任务包装（模式 A）
│   ├── setup_secrets.py            # 密钥文件 → GitHub Secrets 同步
│   ├── requirements.txt            # 采集/加工依赖
│   └── requirements-semantic.txt   # 语义检索可选依赖（本地）
├── raw/                            # 原始素材（只读，仅 status 可被 AI 修改）
├── wiki/                           # 个人学习笔记（只读，仅用户修改）
├── expand/                         # AI 加工产物：条目 + index/log/知识图谱/自动化文档
        ...
        run: |
          ...
          if [ -n "$(git status --porcelain)" ]; then
            ...
            gh pr create --base main --head "$BRANCH" \
              --title "AI 加工 $(date +%F)" \
              --body "本 PR 由 ingest 流程自动生成，请 review 后合并。变更摘要见 expand/log.md。"
          fi
```
...
| 任务 | 时间 | 指令模板 | 输出 | 状态 |
|------|------|----------|------|------|
| 周加工 | 每周一 06:30 | 「摄入 `raw/` 中所有 `status=pending` 的素材，按 agents.md 规则入库」 | expand 条目更新 + 变更摘要 | ✅ 已落地（任务计划程序） |
| 日巡检 | 每天 09:00 | 「检查知识库：断链 / 孤立 / 重复 / 积压，报告并修复可自动修复项」 | 巡检报告（写入 log.md） | ✅ 已落地（任务计划程序） |
| 手动摄入 | 随时 | 「摄入 `D:\note\raw\xxx.md`」 | 单条入库 | ✅ 天然支持 |

### 4.3 与 Actions 的协同与互斥

1. **互斥依据**：素材 `status` 状态机——Codex 加工完成即置 `done`；Actions 加工只处理 `pending`，天然互斥。
2. **冲突规避**：Actions 只在 `raw/` 新增文件、写 `expand/`，不写 `wiki/`；Codex 写 `expand/` 前先 `git pull`。
3. **双跑保护**：`content_hash` 幂等——即使两端同时加工同一素材，第二次写入会被跳过。
4. **审阅分工**：模式 A 由用户抽查 Codex 变更摘要；模式 B 由 PR review 把关。

### 4.4 深度技术笔记模板（AI 条目标准）

摄入外部素材时，AI 生成条目按「深度技术笔记模板」加工（详见 `agents.md`「知识条目格式」，取代六维框架）：

| 规则 | 要求 |
|------|------|
| 强制补全 | 素材浅薄时主动补充外部权威知识并**融入正文**，句末用 `[补充]` 标注 |
| 人话解释 | 核心概念给"一句话人话解释"（类比 / 底层逻辑） |
| 代码规范 | 最小可运行示例 + 语言 / 框架 / 版本标注 |
| 工程视角 | 性能 / 安全 / 成本 / 异常处理 |
| 排版 | 表格 / 列表 / 加粗，拒绝长段落 |

输出结构：`## 本周主题` → `一句话总结` → `核心概念拆解` → `架构与方案对比` → `代码与实操速查` → `避坑清单` → `知识关联地图` → `素材盲区与知识增量` → `行动清单`。

> 个人手写笔记仍保持原文不变；模板只用于 AI 从外部素材生成的新条目。管道已接入 Bing Search API 联网检索（`BING_SEARCH_API_KEY`），`[补充]` 以真实检索结果为准并标注来源 URL；未配置密钥时回退模型内部知识。
...
- [x] 清理待办：重命名含 `#` 的文件名（4 个已完成）；合并 Free-fs 重复（已完成，保留「网盘项目的解析」版）；空笔记已在 index 标记
...
- [x] `scripts/lint.py`：断链 / 孤立 / 重复 / 积压 / index 同步 / 空笔记六项检查（首跑全绿）
- [x] `scripts/scan_secrets.py`：凭据正则扫描（本地验证 0 命中）
- [x] `lint.yml`（每日 08:30）+ `scan-secrets.yml`（push/PR 时执行）已创建，待推送后生效
- [x] Obsidian Git 插件已就位（community-plugins.json 已启用）

**验收**：一次跑通，报告正确；凭据扫描对测试用例命中。

### P2 采集与过滤（2-3 天）

- [x] `scripts/collect.py`：GitHub Search / HN / ArXiv 三源采集 + 关键词粗滤 + URL 去重（本地实测通过）
- [x] `scripts/fetch_full.py`：全文抓取（GitHub README / 文章正文 / arXiv / YouTube / B站字幕 / PDF）+ DeepSeek 中文翻译
- [x] `scripts/filter.py`：标题去重 + LLM 三维打分（DeepSeek，失败回退启发式）
- [x] `collect.yml` 上线（每周一 06:00：采集 → 全文翻译 → 打分 → 提交；LLM 配置走 Secrets）
- [ ] 视频源待实测：YouTube / B 站字幕抓取逻辑已实现，待真实视频链接验证

**验收**：周一自动产出带中文全文的 `raw/` 素材，通过率 20-50%。

### P3 AI 加工（2-3 天）

- [x] 六维加工框架落地：agents.md 条目格式升级 + 首批 10 条素材按框架重写（2026-08-04）
- [x] 深度技术笔记模板升级：agents.md / ingest.py Prompt / 设计文档同步更新（2026-08-04，取代六维框架）
- [x] 模式 B：`scripts/ingest.py` + `count_pending.py` + `ingest.yml`（DeepSeek 六维生成 → 分支 → PR 人工审阅；本地 dry-run 验证通过）
- [x] 状态机 + 幂等：raw `status → processed` + `processed_hash`，重复素材跳过
- [x] 模式 A：配置 Codex 定时任务（周加工 + 日巡检）——Windows 任务计划程序 `Codex-KB-Weekly-Ingest`（周一 06:30）与 `Codex-KB-Daily-Lint`（每天 09:00），经 `scripts/codex_task.ps1` 调用 Codex CLI

**验收**：`pending` 素材能变成符合六维框架的 wiki 条目 + 双向链接 + index/log 更新；重复触发无重复写入。

### P4 拓展（持续）

- [x] `weekly-report.yml`（每周五 19:00）+ 失败告警（F08 / F09，`failure-notify.yml`）
- [x] Dataview 动态索引 + Templater 模板（F10：`expand/动态索引.md` + `templates/新条目模板.md`，插件已安装）
- [x] 语义检索（F11：`scripts/semantic_search.py`，bge-small-zh embedding + TF-IDF 降级，SQLite 本地索引）
- [x] RSSHub 国内源接入（掘金分类 / 知乎热榜；公共实例不稳定，建议自建后配置 `RSSHUB_BASE` Secret）
...
- [x] P0-P2：git / 巡检 / 安全 / 采集管道（已完成，见路线图）
- [x] P3：Codex 定时任务接入（Windows 任务计划程序：周加工周一 06:30 + 日巡检每天 09:00）
- [x] P4：周报 / 告警 / Dataview·Templater / 语义检索 / RSSHub 接入
- [ ] 用户操作：填写 `私密/API密钥.md` 后运行 `python scripts/setup_secrets.py`（BING_SEARCH_API_KEY / GH_PAT / NOTIFY_WEBHOOK）
- [ ] 安全：吊销 `私密/密码管理.md` 中的 GitHub Token 与阿里云 AccessKey
- [ ] 建议：自建 RSSHub（docker）以稳定国内源；重启 Obsidian 以加载 Dataview / Templater 插件
