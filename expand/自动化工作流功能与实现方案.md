...
- **中文翻译**：抓取内容经可插拔翻译后端翻译为简体中文（`scripts/translator.py`：本地 MarianMT 零成本 → Google 免费 → DeepSeek LLM 兜底，`TRANSLATE_BACKEND` 可切换），素材以中文 Markdown 保存（frontmatter：`source / url / title / collected / status / score / tags / translated`）。
...
│   ├── translator.py               # 可插拔翻译后端（本地 MarianMT / Google / LLM 兜底）
...
- [x] `scripts/fetch_full.py`：全文抓取（GitHub README / 文章正文 / arXiv / YouTube / B站字幕 / PDF）+ `translator.py` 可插拔翻译（本地模型 / Google / LLM 兜底）
...
- [x] `weekly-report.yml` + 失败告警（F08 / F09，2026-08-04 落地）
- [x] Dataview 动态 index + Templater 模板（F10，2026-08-04 落地）
- [x] 语义检索（F11，2026-08-04 落地，sentence-transformers + SQLite）
- [x] RSSHub 国内源（掘金 / 知乎 / CSDN，2026-08-04 落地，`RSSHUB_BASE` 可配置）
- [ ] 可选：self-hosted runner（解决国内源网络问题）

**验收**：周报自动产出；语义检索命中相关条目；告警可到达。

### P5 一致性门禁（Track 1，2026-08-09 落地）

> 从「巡检报告」升级为「提交断言」：知识库结构损坏时本地提交 + CI 双双拦截。

- [x] `scripts/check_consistency.py`：K1-K7 不变量检查（raw 状态机 / index 计数 / frontmatter / 双链 / 表格 / 孤立）
- [x] `.githooks/pre-commit`：本地提交前触发，`git config core.hooksPath .githooks` 启用
- [x] `.github/workflows/consistency.yml`：CI 门禁，job 名固定 `consistency / check`
- [ ] 在 GitHub 分支保护中把 `consistency / check` 设为必需检查（需管理员权限，push 后配置）

**验收**：违规条目本地提交与 PR 均被拦截，报告含具体修复指引。
