# farion1231/cc-switch — A cross-platform desktop All-in-One assistant for Claude Code, Codex, OpenCode, OpenClaw, Grok Build & Hermes Agent
...
- **来源：** https://github.com/farion1231/cc-switch （唯一官方网站：https://ccswitch.io）
- **发布：** 仓库创建于 2025-08-04；本 README 采集于 2026-09-12（`main` 分支，最后推送 2026-09-11T15:33:41Z）
- **性质：** 开源仓库主 README（英文正文 Markdown 约 47KB、602 行；含 logo/徽章 HTML、32 行赞助商表格、FAQ、折叠的架构总览/开发指南/项目结构详情）
- **抓取：** 2026-09-12，GitHub API `repos/farion1231/cc-switch/readme`（`Accept: application/vnd.github.raw+json`）取得权威正文；仓库元数据取 `repos/farion1231/cc-switch`、最新 release 取 `repos/farion1231/cc-switch/releases/latest`；官方简体中文版 `README_ZH.md` 另存 `/tmp` 仅用于术语与专有名词交叉核对，未作为译文来源
- **仓库指标（2026-09-12）：** stars 132,370 / forks 9,129 / open issues 2,663 / watchers（subscribers）250；主语言 Rust；LICENSE 为 MIT；最新 release v3.20.3（发布于 2026-09-11T16:10:52Z）；topics 含 ai-tools、claude-code、codex、desktop-app、grok、hermes-agent、mcp、open-source、openclaw、opencode、provider-management、rust、skills、tauri、wsl-support 等
- **官方中文版：** 仓库提供 `README_ZH.md`（简体中文）；本译文以英文 `README.md` 为唯一原文，中文版仅用于术语/专有名词交叉核对
