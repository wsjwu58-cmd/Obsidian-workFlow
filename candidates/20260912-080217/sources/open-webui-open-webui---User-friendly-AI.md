...
- **发布：** 仓库创建于 2023-10-06；本 README 采集于 2026-09-12（`main` 分支，最后推送 2026-09-10）
- **性质：** 开源仓库主 README（英文正文 Markdown 约 17KB / 261 行；含状态徽章、站点链接与演示图）
- **抓取：** 2026-09-12，GitHub API `repos/open-webui/open-webui/readme`（`Accept: application/vnd.github.raw+json`）取得权威正文；仓库元数据取自 GitHub API `repos/open-webui/open-webui`
- **仓库指标（2026-09-12）：** stars 151,673 / forks 22,191 / open issues 305 / watchers（subscribers）660；主语言 Python；topics 含 ai、llm、llm-ui、llm-webui、ollama、openai、rag、mcp、self-hosted、webui 等
- **许可证：** 自定义（GitHub 标记 `NOASSERTION`）—— Open WebUI License，附带保留 "Open WebUI" 品牌的附加要求，另含历史贡献沿用原许可证；详见 `LICENSE` / `LICENSE_HISTORY`
- **官方中文版：** 仓库 README 未提供官方简体中文版（语言项目通过 i18n 在应用内本地化），故本译文以英文 `README.md` 为唯一原文
